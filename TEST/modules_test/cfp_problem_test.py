...
def test_create_problem_test():
    ...
    pr = Problem(contestId=1, problemset_name='test problemset', index='a', name='test problem', problem_type=ProblemType.CF_PROGRAMMING, points=float(100), rating=100, tags=tags)

    assert pr.contest_id == 1
    assert pr.index == 'a'
    assert pr.problemset_name == 'test problemset'
    assert pr.name == 'test problem'
    assert pr.problem_type == ProblemType.CF_PROGRAMMING
